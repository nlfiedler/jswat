 ...
 *
 * $Id: TreeBuilder.java 14 2007-06-02 23:50:55Z nfiedler $
 *
 ********************************************************************/
...
class TreeBuilder extends AnalysisAdapter {
    ...
    private int ARGUMENT = 1, OPERATOR = 2, APPEND = 3;
     ...
     */
    protected void setError(int error, Token token, String add) {
        setError(Errors.getMessage(error) + ": " + token.getText()
                 + " @ " + token.getPos() + ": " + add);
    } // setError
    ...
    public void caseTIdentifier(TIdentifier token) {
        ...
        if (searchState == APPEND) {
            ...
            Node node = (Node) argumentStack.peek();
            if (node instanceof JoinableNode) {
                Node opr = (Node) operatorStack.peek();
    ...
    public void caseTRBracket(TRBracket node) {
        ...
        Node index = (Node) argumentStack.pop();
        Node name = (Node) argumentStack.pop();
