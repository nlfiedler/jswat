 ...
 *
 * $Id: SourcesView.java 15 2007-06-03 00:01:17Z nfiedler $
 */
...
public class SourcesView extends AbstractView
    ...
    private static boolean isEmpty(FileObject fo, boolean recurse) {
        if (fo != null) {
            ...
            for( int i = 0; i < kids.length; i++ ) {
                if (!kids[i].isFolder() &&
                        VisibilityQuery.getDefault().isVisible(kids[i])) {
                    return false;
                } else if (recurse && !isEmpty(kids[i])) {
                    return false;
                }
            }
        }
     ...
     * @author  Nathan Fiedler
     */
    private static class SourceRootNode extends BaseNode {
        ...
        public SourceRootNode(FileObject root) {
            ...
            setIconBaseWithExtension(
                    "com/bluemarsh/jswat/ui/resources/views/PackageRoot.gif");
            sourceRoot = root;
        ...
        protected void performAction(Node[] activatedNodes) {
            ...
            for (String path : paths) {
                ...
                if (!file.exists() || !file.canRead()) {
                    // How likely is that when the user chose the file from
                    // a JFileChooser widget?
                    continue;
                }
