...
/**
 * Sets a breakpoint to stop when classes are loaded and unloaded.
 *
 * @author Nathan Fiedler
 */
public class ClassBreakpointCommand extends AbstractCommand {
    ...
    public void perform(CommandContext context, CommandArguments arguments)
        ...
        while (arguments.hasMoreTokens()) {
            ...
            } else if (token.equals("prepare")) {
                onUnload = false;
            } else if (token.equals("unload")) {
                onPrepare = false;
            } else {
                filter = token;
                break;
        ...
        Breakpoint bp = bf.createClassBreakpoint(filter, onPrepare, onUnload);
        bp.setEnabled(false);
        bp.setSuspendPolicy(suspendPolicy);
        bp.setEnabled(true);
        BreakpointManager bm = BreakpointProvider.getBreakpointManager(session);
        bm.addBreakpoint(bp);
        writer.println(NbBundle.getMessage(ClassBreakpointCommand.class,
                "CTL_classbrk_Added"));
    }
}
